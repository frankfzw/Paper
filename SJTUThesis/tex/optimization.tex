...
在这里我们比较了三种不同的调度算法在调度不同轮次的任务时性能，包括随机调度算法，Spark默认调度算法（FIFO）\cite{apachespark}以及我们提出的基于启发式的最小堆算法。
...
图\ref{fig:range_pre_sample}则展示了在数据倾斜度较高的数据集中使用范围分区函数进行数据分块的情况下，线性回归模型和采样预测的情况。
