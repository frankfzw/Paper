...
这些分块的数据即在shuffle阶段通过网络传输的数据。他们往往会在分块结束之后被写入本地磁盘。
...
有相关研究表明，Yahoo!公司中60\%的MapReduce应用和Facebook公司中20\%的MapReduce应用中存在大量的shuffle数据\cite{shufflewatcher}。
对于这些存在大量shuffle的应用中，shuffle传输所带来的延迟甚至会成为整个应用完成时间的瓶颈。
比如在一份Facebook公司公开的MapReduce任务日志中，shuffle过程的平均占用了整个应用完成时间的33\%。
对于那些存在大量shuffle的应用，shuffle的开销甚至最多占用了整个应用完成时间的70\%\cite{managing}。
...
一个map任务的结束是在他完成了shuffle write之后。
