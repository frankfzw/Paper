...
For each predicted reduce size, a percentage array of total data composition among each map output is calculated. This array can assist the scheduling to reduce the network traffic while acheiving the uniform load on each node. With this composition array and the predicted size of reduce, we present a heuristic MinHeap as the scheduling algorithem for single shuffle.
\begin{minipage}{\linewidth}
\begin{algorithm}[H]
\caption{Heuristic MinHeap Scheduling for Single Shuffle}
\label{hminheap}
	\begin{algorithmic}[1]
	\Procedure{doSchedule}{$maps, hosts, p\_reduces$}
		...
		\ElsIf{$nor\geq$ rand$\left(0,1\right)$}
