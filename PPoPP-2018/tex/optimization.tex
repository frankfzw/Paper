...
SCache provides a disk-write-like API named $putBlock$ to handle the storage of partitioned shuffle data blocks that produced by a map task.
...
And the size of each reduce task $reduceSize_i = \sum_{k=0}^{s} \sum_{j=0}^{m} {BlockSize_{ji}}$. 
...
The $k$ and $b$ can be determined using the observed $inputSize_j$ and $BlockSize_{ji}$.
}
\begin{figure}
	\centering
...
We present two particular examples with 20 tasks repectively in Figure \ref{fig:hash_pre} and Figure \ref{fig:range_pre_sample}. 
\hlyellow{
...
\hlyellow{
For the pre-scheduling itself (i.e., the first $while$ in Algorithm} \ref{hminheap}), \hlyellow{the algorithm maintains a min-heap to simulate the load of each node 
...
After scheduling, if the new $assigned\_id$ of a reduce did not equal the original one, a re-shuffle would be triggered to transfer data to the new host. 
