...
That is, for one reduce task, almost all of the input date are produced by a particular map task (e.g., the observed map tasks only produce data for reduce tasks 0-5 in Figure} \ref{fig:fig:range_pre_sample}).
\hlyellow{
...
\fi
\hlyellow{
But as we mentioned before, the ignorance of data locality can introduce extra network transfer, especially in the extreme data locality case (e.g., Figure} \ref{fig:fig:range_pre_sample}). 
\hlyellow{
...
\fi
\hlyellow{
For the pre-scheduling itself(i.e., the first $while$), algorithm maintains a min-heap to simulate the load of each node 
