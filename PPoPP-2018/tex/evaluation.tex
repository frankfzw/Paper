...
Then we use a recognized shuffle intensive benchmark --- Terasort} \cite{spark-tera} \hlyellow{to evaluate the pre-scheduling scheme of SCache.} 
\begin{figure*}
	\centering
...
It is because that those queries contains shuffle intensive operations such as "groupby", "union", etc.
...
\hlyellow{
Since the shuffle data is short-lived, write-once, and read-once, the central controller of SCache does not have to collect and store complex metadata. 
Meanwhile, most of the optimizations, such as fetching and storing shuffle data, are finished by worker independently. 
So the largest cost of the central controller is produced by pre-scheduling algorithm, which make it unlikely to become the bottleneck of the whole system.
}
