...
Shuffle read starts at the beginning of reduce tasks. The data consumed by reduce tasks will be fetched from both remote storage during shuffle read.
}
...
To find the answers, we run some typical Spark applications in a 5-node \texttt{m4.xlarge} EC2 cluster, and measure the CPU utilization, I/O throughput, and tasks execution information of each node. 
Here we present the trace of one node running Spark \textit{GroupByTest} in Figure \ref{fig:util} as an example. 
