...
Besides, we analyze the design and implementation of shuffle in other DAG frameworks.
}
...
\hlyellow{
Note that this defect exists in not only Spark} \cite{apachespark}, but also Hadoop MapReduce \cite{hadoop} and Apache Tez \cite{tez}. 
...
\hlyellow{
the data transfer puts a great deal of stress on network bandwidth, which may results in network congestion and further slows down the network transfer.
}
\hlyellow{
...
\hlyellow{
Unfortunately, most of the DAG frameworks still store shuffle data on disks }(e.g., Spark \cite{apachespark}, Hadoop MapReduce \cite{hadoop}, Dryad \cite{dryad}, etc).
...
\hlyellow{
That is, for a cluster with $n$ number of slots, the number of tasks should be $n \times k (k \geq 1)$. 
}
\ifrevision
\reversemarginpar
\marginnote{60E}
...
To mitigate the shuffle overhead, we propose an optimization that starts shuffle read ahead of reduce stage to overlap the I/O operations in multi-round DAG computing tasks, and uses memory to store the shuffle data. 
}
